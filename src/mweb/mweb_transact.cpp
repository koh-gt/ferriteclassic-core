...
TxType MWEB::GetTxType(const std::vector<CRecipient>& recipients, const std::set<CInputCoin>& input_coins)
{
    ...
    if (recipients.front().IsMWEB()) {
        ...
        if (std::any_of(input_coins.cbegin(), input_coins.cend(), is_fecc)) {
            return TxType::PEGIN;
        } else {
        ...
        // Otherwise, if there are no MWEB inputs, it's a simple FECC-to-FECC transaction.
        if (std::any_of(input_coins.cbegin(), input_coins.cend(), is_mweb)) {
            return TxType::PEGOUT;
        } else {
            return TxType::FECC_TO_FECC;
        }
    }
}
...
uint64_t MWEB::CalcMWEBWeight(const MWEB::TxType& mweb_type, const bool change_on_mweb, const std::vector<CRecipient>& recipients)
{
    ...
    if (mweb_type != MWEB::TxType::FECC_TO_FECC) {
...
void Transact::AddMWEBTx(InProcessTx& new_tx)
{
    ...
    // Lookup the change paid on the FECC side
    CAmount fecc_change = 0;
    if (new_tx.change_position != -1) {
        assert(new_tx.tx.vout.size() > (size_t)new_tx.change_position);
        fecc_change = new_tx.tx.vout[new_tx.change_position].nValue;
    }

    // Calculate pegin_amount
    boost::optional<CAmount> pegin_amount = boost::none;
    CAmount fecc_input_amount = std::accumulate(
    ...
    if (fecc_input_amount > 0) {
        const CAmount fecc_fee = new_tx.total_fee - new_tx.mweb_fee;
        assert(fecc_fee <= fecc_input_amount);
        pegin_amount = (fecc_input_amount - (fecc_fee + fecc_change));
    }

    // Add Change
    if (new_tx.change_on_mweb) {
        receivers.push_back(BuildChangeRecipient(new_tx, pegin_amount, fecc_change));
    }
...
mw::Recipient Transact::BuildChangeRecipient(const InProcessTx& new_tx, const boost::optional<CAmount>& pegin_amount, const CAmount& fecc_change)
{
    ...
    CAmount change_amount = (pegin_amount.value_or(0) + mweb_input_amount) - (recipient_amount + new_tx.mweb_fee + fecc_change);
    if (change_amount < 0) {
