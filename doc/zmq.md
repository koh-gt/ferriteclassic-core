...
The FerriteClassic Core daemon can be configured to act as a trusted "border
router", implementing the ferriteclassic wire protocol and relay, making
...
The ZeroMQ feature in FerriteClassic Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building ferriteclassicd:
...
For instance:
    ...
    $ ferriteclassicd -zmqpubhashtx=tcp://127.0.0.1:28332 \
               -zmqpubhashtx=tcp://192.168.1.2:28332 \
               -zmqpubrawtx=ipc:///tmp/ferriteclassicd.tx.raw \
...
These options can also be provided in ferriteclassic.conf.
...
The effecctive TCP keepalive values are managed through the underlying
...
From the perspective of ferriteclassicd, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into ferriteclassicd directly. Furthermore, no information is
...
using. FerriteClassicd appends an up-counting sequence number to each
