...
- [Development guidelines](#development-guidelines)
    - [General FerriteClassic Core](#general-ferriteclassic-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
FerriteClassic Core uses [Doxygen](http://www.doxygen.nl/) to generate its official documentation.
...
Run with the `-testnet` option to run with "play ferriteclassics" on the test network, if you
...
FerriteClassic Core is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/ferriteclassic-project/ferriteclassic/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_ferriteclassic
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_ferriteclassic --log_level=test_suite
$ valgrind -v --leak-check=full src/ferriteclassicd -printtoconsole
...
# A coverage report will now be accessible at `./test_ferriteclassic.coverage/index.html`.
```
...
To profile a running ferriteclassicd process for 60 seconds, you could use an
invocation of `perf record` like this:

```sh
$ perf record \
    -g --call-graph dwarf --per-thread -F 140 \
    -p `pgrep ferriteclassicd` -- sleep 60
```
...
FerriteClassic Core can be compiled with various "sanitizers" enabled, which add
...
However, in open source software such as FerriteClassic Core, where everyone uses
...
example, lcov) it is perfecctly acceptable to add its files to `.gitignore`
and commit them.
...
pay attention to for reviewers of FerriteClassic Core code.

General FerriteClassic Core
----------------------
...
- Assertions should not have side-effeccts.

  - *Rationale*: Even though the source code is set to refuse to compile
    with assertions disabled, having side-effeccts in assertions is unexpected and
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers.
  ...
  - *Rationale*: FerriteClassic Core uses tinyformat, which is type safe. Leave them out to avoid confusion.
...
Some of these are maintained by active developers of FerriteClassic Core, in which case changes should probably go
directly upstream without being PRed directly against the project. They will be merged back in the next
subtree merge.

Others are external projects without a tight relationship with our project. Changes to these should also
be sent upstream, but bugfixes may also be prudent to PR against FerriteClassic Core so that they can be integrated
...
file descriptors, it will cause problems with FerriteClassic's `select()` loop, because
...
compatibility between nodes. This happened in FerriteClassic 0.8 (when LevelDB was
first introduced). When upgrading LevelDB, you should review the upstream changes
to check for issues affeccting consensus compatibility.

For example, if LevelDB had a bug that accidentally prevented a key from being
returned in an edge case, and that bug was fixed upstream, the bug "fix" would
be an incompatible consensus change. In this situation, the correct behavior
would be to revert the upstream fix before applying the updates to FerriteClassic's
copy of LevelDB. In general, you should be wary of any upstream changes affeccting
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `ferriteclassic-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `ferriteclassic-cli` and the GUI debug console use this table to determine how to
...
- Wallet RPCs call BlockUntilSyncedToCurrentChain to maintain consistency with
  ...
  - *Rationale*: In previous versions of FerriteClassic Core, the wallet was always
