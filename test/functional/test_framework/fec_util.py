...
# Copyright (c) 2022-2023 The FerriteClassic Core developers
...
def create_non_hd_wallet(chain, options):    
    ...
    conf_file = os.path.join(data_dir, 'ferriteclassic.conf')
    ...
    v15_node = TestNode(
        ...
        timewait=60,
        timeout_factor=1.0,
        bitcoind=os.path.join(bin_dir, 'ferriteclassicd'),
        bitcoin_cli=os.path.join(bin_dir, 'ferriteclassic-cli'),
        version=version,
        coverage_dir=None,
        cwd=options.tmpdir,
        extra_args=["-usehd=0"],
